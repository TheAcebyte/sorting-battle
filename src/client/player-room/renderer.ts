import { easeOutExpo, Easing } from "@shared/easings";
import { Interpolator } from "@shared/interpolator";
import { Bar } from "@shared/types";

type InterpolatedBar = Bar & { index: Interpolator };
...
export class Renderer {
  ...
  public updateBars(bars: Bar[]) {
    ...
    for (let i = 0; i < n; ++i) {
      ...
      if (this.barMap.has(bar.barId)) {
        const { index } = this.barMap.get(bar.barId)!;
        index.set(i, this.options.swapDuration);
        temp.set(bar.barId, { ...bar, index });
      } else {
        const index = new Interpolator(i, this.options.easing);
        temp.set(bar.barId, { ...bar, index });
      }
    }
  ...
  private drawBar(bar: InterpolatedBar) {
    const { color, playerColor, width, heightStep, gap } = this.options;
    const n = this.barMap.size;
    const listWidth = n * width + (n - 1) * gap;
    const startX = (this.canvas.width - listWidth) / 2;
    const height = bar.height * heightStep;
    const x = startX + bar.index.get() * (width + gap);
